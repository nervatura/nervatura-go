...
func (ds *SQLDriver) createTableFields(sqlString, fieldname, indexName string, field nt.MF) string {
	...
	if field.Default != nil && field.Default != "nextnumber" {
		...
		case int64:
			sqlString += " " + "DEFAULT" + " " + strconv.FormatInt(v, 10)
		case float64:
			sqlString += " " + "DEFAULT" + " " + strconv.FormatFloat(v, 'f', -1, 64)
		case string:
			sqlString += " " + "DEFAULT" + " " + v
		}
	}
...
func (ds *SQLDriver) QueryKey(options IM, trans interface{}) (result []IM, err error) {
	...
	keys := map[string]func(options IM) (string, IL, error){
		...
		"user_guest": func(options IM) (string, IL, error) {
			params := IL{}
			sqlString =
				`select e.id, e.username, e.empnumber, ug.id as usergroup, ug.groupvalue as scope, 
				case when dg.groupvalue is null then '' else dg.groupvalue end as department
...
func (ds *SQLDriver) GetIDfromRefnumber(options IM) (id int64, info IM, err error) {
	return
}
