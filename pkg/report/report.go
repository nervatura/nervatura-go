...
func (rpt *Report) createDatagrid(gridElement *Datagrid) bool {
	...
	headerOptions := IM{
		...
		"columns":      make([]IM, 0),
		"columnsWidth": float64(0), "gridWidth": float64(0), "multiline": false}
	...
	footerOptions := IM{
		...
		"extend":          headerOptions["extend"], "multiline": false}
...
func (rpt *Report) createLine(v *HLine) {
	width := float64(0)
	pageWidth, _ := rpt.pdf.GetPageSize()
...
/*
func (rpt *Report) getXMLElements(xdata etree.Element) (PageItem, error) {
	el, err := rpt.getPageItem(xdata.Tag)
	if err != nil {
		return el, err
	}
	for index := 0; index < len(xdata.Attr); index++ {
		if err = el.setPageItem(xdata.Attr[index].Key, rpt.parseValue(propMap[strings.ToLower(xdata.Attr[index].Key)], xdata.Attr[index].Value)); err != nil {
			return el, err
		}
	}
	if xdata.Tag == "html" {
		if err = el.setPageItem("html", rpt.parseValue("Html", xdata.Text())); err != nil {
			return el, err
		}
	}
	if columns := xdata.SelectElement("columns"); columns != nil {
		colChilds := columns.ChildElements()
		for colIndex := 0; colIndex < len(colChilds); colIndex++ {
			switch colChilds[colIndex].Tag {
			case "cell", "image", "barcode", "separator", "column":
				el2, err := rpt.getPageItem(colChilds[colIndex].Tag)
				if err != nil {
					return el2, err
				}
				for attrIndex := 0; attrIndex < len(colChilds[colIndex].Attr); attrIndex++ {
					attr := colChilds[colIndex].Attr[attrIndex]
					if err = el2.setPageItem(attr.Key, rpt.parseValue(propMap[strings.ToLower(attr.Key)], attr.Value)); err != nil {
						return el, err
					}
				}
				if xdata.Tag == "row" {
			...
			default:
				return el, errors.New(invalidErr("Columns", colChilds[colIndex].Tag))
			}
		}
	}
	return el, nil
...
func (rpt *Report) LoadDefinition(xmlString string) (bool, error) {
	...
	if err := xdoc.ReadFromString(xmlString); err != nil {
		return false, err
	}

	if report := xdoc.SelectElement("template").SelectElement("report"); report != nil {
		for index := 0; index < len(report.Attr); index++ {
			if err := rpt.SetReportValue(report.Attr[index].Key, rpt.parseValue(propMap[strings.ToLower(report.Attr[index].Key)], report.Attr[index].Value)); err != nil {
				return false, err
			}
		}
	}
	if header := xdoc.SelectElement("template").SelectElement("header"); header != nil {
		headerChilds := header.ChildElements()
		for index := 0; index < len(headerChilds); index++ {
			xElement := headerChilds[index]
			switch xElement.Tag {
			case "row", "vgap", "hline":
				el, err := rpt.getXMLElements(*xElement)
				if err != nil {
					return false, err
				}
				rpt.header = append(rpt.header, el)
			default:
				return false, errors.New(invalidErr("Header", xElement.Tag))
			}
		}
	}
	if details := xdoc.SelectElement("template").SelectElement("details"); details != nil {
		detailsChilds := details.ChildElements()
		for index := 0; index < len(detailsChilds); index++ {
			xElement := detailsChilds[index]
			switch xElement.Tag {
			case "row", "vgap", "hline", "html", "datagrid":
				el, err := rpt.getXMLElements(*xElement)
				if err != nil {
					return false, err
				}
			...
			default:
				return false, errors.New(invalidErr("Details", xElement.Tag))
			}
		}
	}
	if footer := xdoc.SelectElement("template").SelectElement("footer"); footer != nil {
		footerChilds := footer.ChildElements()
		for index := 0; index < len(footerChilds); index++ {
			xElement := footerChilds[index]
			switch xElement.Tag {
			case "row", "vgap", "hline":
				el, err := rpt.getXMLElements(*xElement)
				if err != nil {
			...
			default:
				return false, errors.New(invalidErr("Footer", xElement.Tag))
			}
		}
	}
	if data := xdoc.SelectElement("template").SelectElement("data"); data != nil {
		dataChilds := data.ChildElements()
		for dataIndex := 0; dataIndex < len(dataChilds); dataIndex++ {
			if len(dataChilds[dataIndex].Attr) > 0 {
				rpt.data[dataChilds[dataIndex].Tag] = SM{}
				for attrIndex := 0; attrIndex < len(dataChilds[dataIndex].Attr); attrIndex++ {
					elx := dataChilds[dataIndex].Attr[attrIndex]
					rpt.data[dataChilds[dataIndex].Tag].(SM)[elx.Key] = elx.Value
				}
			} else {
				itemChilds := dataChilds[dataIndex].SelectElements(dataChilds[dataIndex].Tag)
				if len(itemChilds) > 0 {
					dkey := dataChilds[dataIndex].Tag
					rpt.data[dkey] = make([]SM, 0)
					for itemIndex := 0; itemIndex < len(itemChilds); itemIndex++ {
						dRow := SM{}
						for attrIndex := 0; attrIndex < len(itemChilds[itemIndex].Attr); attrIndex++ {
							dRow[itemChilds[itemIndex].Attr[attrIndex].Key] = itemChilds[itemIndex].Attr[attrIndex].Value
						}
						rpt.data[dkey] = append(rpt.data[dkey].([]SM), dRow)
					}
				} else {
					txt := dataChilds[dataIndex].Text()
					rpt.data[dataChilds[dataIndex].Tag] = txt
				}
			}
		}
	}
				...
				for index := 0; index < len(dValue.([]interface{})); index++ {
					...
					for key, value := range jRow.(IM) {
						if svalue, valid := value.(string); valid {
							...
							return errors.New(invalidErr("value", key))
						}
					}
					rpt.data[dKey] = append(rpt.data[dKey].([]SM), dRow)
				}
